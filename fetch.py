...
def send_notification(message):
    """
    Function to send a notification (or a message to a notification icon).
    """
    # You can integrate actual notification logic here, such as:
    # - Sending an email
    # - Pushing a mobile notification
    # - Displaying a message on the frontend UI via WebSockets
        ...
        try:
            ...
            try:
                for i in range(len(timestamps)):
                    ...
                    # # Check if cloud cover exceeds threshold for notification
                    # if total_cover >= 75:  # Example: 75% or more cloud cover
                    #     send_notification(f"Cloud cover is high ({cloud_cover_total}%) at coordinates ({latitude}, {longitude}).")
            ...
            # Check if current cloud cover exceeds threshold for notification
            if current_total_cover >= 75:  # Example: 75% or more cloud cover
                send_notification(f"Current cloud cover is high ({current_total_cover}%) at coordinates ({latitude}, {longitude}).")
